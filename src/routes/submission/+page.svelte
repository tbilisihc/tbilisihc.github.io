<script>
	...
	// --- API Endpoint ---
	const PUBLIC_SUBMISSIONS_API = 'https://api.tbilisihc.andrinoff.com/.netlify/functions/read-public-submissions';
	...
	<div class="min-h-screen p-4 sm:p-6 lg:p-8">
		<div class="max-w-7xl mx-auto">
			...
			<main>
				{#if isLoading}
					<!-- Loading State -->
					<div class="flex flex-col items-center justify-center text-gray-400 mt-16">
						<svg
				...
				{:else if error}
					<!-- Error State -->
					<div
						class="flex flex-col items-center justify-center text-red-400 bg-red-900/20 p-8 rounded-lg mt-16"
					>
						<svg
				...
				{:else}
					...
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						{#each submissions as submission (submission.name)}
							<div
								class="bg-gray-600 border border-gray-400 rounded-xl shadow-lg p-6 transform hover:scale-105 transition-transform duration-300 ease-in-out"
							>
								<div class="flex items-center">
									<div class="bg-purple-600/20 p-3 rounded-full mr-4">
										{@html userIcon}
									</div>
									<h3 class="text-xl font-bold text-gray truncate">{submission.name}</h3>
								</div>
							</div>
						{/each}
					</div>
