<script>
  ...
  const ADMIN_LOGIN_API =
    "https://api.tbilisihc.andrinoff.com/.netlify/functions/admin-login";
  const UPDATE_SUBMISSION_API =
    "https://api.tbilisihc.andrinoff.com/.netlify/functions/update-submission";
  const EMAIL_API_URL =
  ...
  async function handleAcceptAndSend() {
    ...
    if (!toEmail || !subject || !body || !password || !submissionId) {
      feedback = {
        message: "Please fill out all fields. Missing ID or Email.",
        isError: true,
      };
      isLoading = false;
      return;
    ...
    try {
      ...
      const updateResponse = await fetch(
        `${UPDATE_SUBMISSION_API}/${submissionId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      if (emailResponse.ok) {
        feedback = {
          message: "Submission accepted and email sent successfully!",
          isError: false,
        };
      } else {
        throw new Error(
          emailResult.message || "Failed to send email after accepting."
        );
      }
    } catch (err) {
...
<div
  class="bg-gray-100 min-h-screen p-4 sm:p-6 lg:p-8 flex items-center justify-center"
>
  <div
    class="w-full max-w-2xl bg-white rounded-xl shadow-lg border border-gray-200"
  >
    <header
    ...
    >
      <div>
        ...
        <p class="text-gray-500">
          To: <span class="font-medium text-gray-700"
            >{toEmail || "No recipient"}</span
          >
          (ID: {submissionId || "N/A"})
        </p>
      </div>
    ...
    <main class="p-6 space-y-4">
      ...
      {#if feedback.message}
        <div
          ...
          class:text-white={feedback.isError ||
            feedback.message.includes("success")}
          class:bg-green-500={!feedback.isError &&
            feedback.message.includes("success")}
          class:bg-red-500={feedback.isError}
          class:text-gray-600={!feedback.isError &&
            !feedback.message.includes("success")}
        >
          {feedback.message}
        </div>
