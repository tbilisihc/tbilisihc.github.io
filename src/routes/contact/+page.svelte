<script>
	...
	async function handleSubmit() {
	  ...
	  const messageContent = `
			${$_("contact_page.discord_notification.title")}
			---------------------------------
			${$_("contact_page.discord_notification.from", { values: { name, email } })}
			${$_("contact_page.discord_notification.subject", { values: { subject: subject || $_("contact_page.discord_notification.no_subject") } })}
			${$_("contact_page.discord_notification.message")}
			${message}
			---------------------------------
		`;
	  ...
	  try {
		...
		if (webhookResponse.ok) {
		  feedbackMessage = $_("contact_page.feedback.success");
		  ...
		  // Nested logic to send email only on successful submission
		  try {
			const emailData = {
			  recipient: email,
			  subject: $_("contact_page.auto_reply.subject"),
			  message: $_("contact_page.auto_reply.message_body"),
			};
		  ...
		  feedbackMessage = $_("contact_page.feedback.error", { values: { status: webhookResponse.status } });
		  feedbackType = "error";
		  console.error("Webhook error:", errorText);
		}
	  } catch (error) {
		feedbackMessage = $_("contact_page.feedback.network_error");
		feedbackType = "error";
	  ...
	  <div class="max-w-5xl mx-auto">
		...
		>
		  <div class="flex flex-col">
			...
			<form on:submit|preventDefault={handleSubmit} class="space-y-6">
			  ...
			  <div>
				<label
				  for="subject"
				  class="block text-sm font-medium text-slate-700">{$_('contact_page.form.subject_label')}</label
				>
				<div class="mt-1">
				  <input
					bind:value={subject}
			  ...
			  <div>
				<label
				  for="message"
				  class="block text-sm font-medium text-slate-700">{$_('contact_page.form.message_label')}</label
				>
				<div class="mt-1">
				  <textarea
					bind:value={message}
			  ...
			  <div>
				...
				>
				  {isSubmitting ? $_('contact_page.form.submitting_button') : $_('contact_page.form.submit_button')}
				</button>
			  </div>
			</form>
			{#if feedbackMessage}
		  ...
		  <div class="flex flex-col space-y-8">
			...
			<div>
			  <h3 class="text-2xl font-bold text-slate-900 mb-4">{$_('contact_page.socials_title')}</h3>
